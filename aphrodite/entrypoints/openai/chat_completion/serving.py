...
from aphrodite.v1.structured_output.schema_features import get_structured_outputs_schema_error
...
class OpenAIServingChat(GenerateBaseServing):
    ...
    async def _create_chat_completion(
        ...
        for i, engine_input in enumerate(engine_inputs):
            ...
            if isinstance(sampling_params, SamplingParams):
                schema_error = get_structured_outputs_schema_error(sampling_params.structured_outputs)
                if schema_error is not None:
                    return self.create_error_response(schema_error)
            ...
            if isinstance(sampling_params, BeamSearchParams):
                ...
                if not request.include_reasoning:
                ...
                elif request._grammar_from_tool_parser or request._tool_exclusion_grammar:
                    ...
                    # nothing to hold the bitmask back for. The same goes for
                    # the `tool_choice="none"` exclusion grammar, which permits
                    # everything but a tool call and so constrains reasoning
                    # only in the one way it must.
                    reasoning_ended = True
    ...
    async def chat_completion_stream_generator(
        ...
        try:
            ...
            log_request_failure(e, "Error in parser creation.")
        ...
        try:
            ...
            log_request_failure(e, "Error in chat completion stream generator.")
            data = self.create_streaming_error_response(e)
            yield f"data: {data}\n\n"
        finally:
            await result_generator.aclose()
